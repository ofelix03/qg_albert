...
class qg_albert(models.Model):
    ...
    state = fields.Selection([('mig_po','Migrate PO'),('conf_po','Confirm PO'),('recv_po','Receive PO'),('mig_so','Migrate SO'),('conf_so','Confirm SO'),('recv_so','Receive SO'), ('invoice_so', 'Invoice SO'), ('predone', 'Post-cleanup'), ('done','Done')],string="Stage",default='mig_po')
    ...
    def process_state(self):
        ...
        if self.state == 'mig_po':
            ...
            self.receive_orders()
            self.state = 'invoice_so'
        elif self.state == 'invoice_so':
            self.create_order_invoices()
            self.state = 'predone'
        elif self.state == 'predone':
            self.predone()
            self.state = 'done'
    ...
    def confirm_orders(self):
        ...
        if self.purchase_order_process:
            ...
            orderNames = []
            for order in orders:
                if self.is_order_confirmed(order.name, self.PURCHASE_ORDER) is not True:
        ...
        if self.sale_order_process:
            ...
            orderNames = []
            for order in orders:
                if self.is_order_confirmed(order.name, self.SALE_ORDER) is not True:
                    orderNames.append(order.name)
    ...
    def create_order_invoices(self):
        print("create_order_invoices()")
        orders = self.env['sale.order'].search([('date_order', '>=', self.date), ('date_order', '<=', self.date)])
        orderIds = orders.ids

        env = api.Environment(self.env.cr, self.env.uid, {'active_ids': orderIds})
        saleAdvancePaymentInvoice = env['sale.advance.payment.inv'].create({"advance_payment_method": "all"})
        saleAdvancePaymentInvoice.create_invoices()

    def predone(self):
        invoice_ids = self.env['account.invoice'].search([('date_invoice', '=', self.date)]).ids
        migrate.move_waybill_number_and_analytic_tags(invoice_ids)
    ...
    def receive_orders(self):
    	if self.purchase_order_process:
            ...
            orderNames = []
            for order in orders:
                if self.is_order_received(order.name, self.PURCHASE_ORDER) is not True:
                    orderNames.append(order.name)
        ...
        if self.sale_order_process:
            ...
            orderNames = []
            for order in orders:
                if self.is_order_received(order.name, self.SALE_ORDER) is not True:
                    orderNames.append(order.name)
    ...
    def save_order_names(self, order_names, order_type, order_state=None):
    	...
    	if order_state == ORDER_STATES['placed']:
            ...
            print("history updated = %s" %order_names)
            ...
            print("history udpated = %s" %order_names)
    	else:
    ...
    def is_order_confirmed(self, order_name, order_type):
        if order_type == self.PURCHASE_ORDER:
            ...
            count = self.env['qg_albert.order_history'].search_count([('name', '=', order_name), ('order_type', '=', self.SALE_ORDER), ('order_confirmed', '=', True)])
            if count == 1:
                return True
            return False
        else:
    ...
    def is_order_received(self, order_name, order_type):
        if order_type == self.PURCHASE_ORDER:
            ...
            count = self.env['qg_albert.order_history'].search_count([('name', '=', order_name), ('order_type', '=', self.SALE_ORDER), ('order_placed', '=', True), ('order_confirmed', '=', True), ('order_received', '=', True)])
            if count == 1:
                return True
            return False
        else:
