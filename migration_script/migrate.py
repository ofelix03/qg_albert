...
field_key_to_model_sale_order_line = {
	'order_line': {
		...
		'filters': 'product_id',
		'selected_fields': 'customer_order_no,truck_numb,invoiced,price_unit,product_id,name,date_planned,account_analytic_id,product_qty,product_uom,product_uom_qty,delay,partner_id,payment_term_id',
		'fields': {
			'product_id': {
				'model': 'product.product',
				'filters': 'name,active',
...
def move_waybill_number_and_analytic_tags(invoice_ids):
	...
	for invoice_id in invoice_ids:
		...
		print("invoice_lines = %s" %invoice_lines)
		# Now let get the willybill_number and tags from v8 for these invoice_lines
		for line in invoice_lines:
			...
			records = odooAdapter.setModel("account.invoice.line").getRecordsWhere(filter=[('customer_order_no', '=', customer_number), ('truck_numb', '=', truck_number), ('quantity', '=', quantity)])
			print("invoice lines = %s" %records)
			if len(records) > 0:
				print("we've got some invoice.lines from v8")
				print("record = %s" %records)
				record = records[0]
				waybill_number = record['waybill_no']
				# account_analytic_id = record['account_analytic_id'][1] #name
				account_analytic_name = record['account_analytic_id'][1] #name
				account_analytic_name  = account_analytic_name.split("/")
				account_analytic_name = account_analytic_name[len(account_analytic_name)-1]

				analytic_records_10 = odooAdapter10.setModel("account.analytic.account").getRecordsWhere(filter=[('name', '=', account_analytic_name)], fields=['id'])

				if analytic_records_10:
					account_analytic_id = analytic_records_10[0]['id']
				else:
					analytic_record = {
						"name": account_analytic_name
					}
					account_analytic_id = odooAdapter10.setModel("account.analytic.account").createRecord(analytic_record)
				...
				odooAdapter10.setModel("account.invoice.line").updateRecord([lineId], update_record)
...
def migrate_sale_orders(orders):
	...
	for order_groups in orders_groupings:
		...
		createdOrder['order_line_ids'] = []
		for line in orderLine:
			truck_number = line['truck_numb']
			customer_order_number = line['customer_order_no']
			print("TruckNumber = %s" %truck_number)
			print("CustomerOrderNo = %s" %customer_order_number)
			odooAdapter10.setModel("sale.order").updateRecord(orderId, {"truck_no": truck_number, "cust_order_no": customer_order_number})
			line.pop("truck_numb")
			line.pop("customer_order_no")
			line.update({"order_id":orderId})
